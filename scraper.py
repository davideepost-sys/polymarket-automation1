...
def compute_score(row):
    ...
    Components (weights sum to 1.0):
      ...
      20% — profit rate (how much profit per $ volume)
      15% — sample confidence (more data = more trustworthy)
      15% — risk-reward ratio (are wins bigger than losses?)
      10% — market diversity (trades across many markets, not one lucky pick)
      15% — hold speed (faster turnover = better for small capital)
    """
    ...
    # Hold speed: 0 days = 1.0, 2 days = 0.5, 4+ days = 0.0
    hold = row.get("Avg_Hold_Days")
    if hold is not None:
        speed = max(0.0, 1.0 - (hold / 4.0))          # linear decay: 0→1, 2→0.5, 4→0
    else:
        speed = 0.3  # unknown = neutral-low
...
def analyze(trader):
    """Phase 2 worker: calculate win rate + market diversity + hold time."""
